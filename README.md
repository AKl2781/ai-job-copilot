...
当前推荐流程支持 Windows + Microsoft Edge，本地解压缩扩展版本为 `0.1.2`。首次使用只需完成一次本地连接安装：
...
之后打开岗位页面，按 `Alt+J` 并点击“分析岗位”；后端未运行时会自动按需启动并在健康检查通过后继续分析。使用结束后可点击“停止本地服务”。首次注册只需做一次，
...
卸载不会删除扩展、项目或 `.env`，也不会停止当前后端。Native Host 不读取 API Key，也不执行扩展传入的路径。
当前仍是开发者本地集成：首次需要复制真实扩展 ID 并注册当前用户 HKCU，不是商店安装包或普通用户一键安装产品，且不支持 macOS/Linux。
...
Windows + Edge 推荐使用上面的扩展内按需启动流程。手动双击 `start_ai_job_copilot.bat` 仍作为诊断入口；该方式需要保持日志窗口打开，使用完毕后可双击 `stop_ai_job_copilot.bat`。
